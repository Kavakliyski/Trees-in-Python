...
def whatitis(tree, s):
    ...
    if position == -1:
        tex = "Няма такъв елемент в дървото"
    elif position == 0:
        tex = s + " е корен в дървото"
    elif tree[position + 1] == "[":
        tex = s + " е родител (вътрешен възел) в дървото"
    elif tree[position + 1] == "," or tree[position + 1] == "]":
        tex = s + " е листо (външен възел) в дървото"
    return tex
